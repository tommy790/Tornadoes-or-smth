...
timer.Create("TIV_WindThink", 0.1, 0, function()
    ...
    -- Apply forces only to TIV vehicles (when not anchored) + released spikes (never all props).
    for _, entry in ipairs(activeVehicles) do
        local state = entry.data and entry.data.state or "idle"
        local isAnchored = (state == "anchored" or state == "lowering" or state == "deploying_spikes")
        local windMPH = TIV.Wind.GetSpeed(entry.veh)

        if not isAnchored and windMPH >= 50 then
