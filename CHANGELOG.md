...
- **Radar Heading Basis Correction**: `sh_custom_config.lua` documents the addon's vehicle-local space as `Forward: +Y (along veh:GetForward())`, but in GMod `Angle:Forward()` is local `+X` and `Angle:Right()` is local `-Y` -- the two halves of that line disagree by 90 degrees. The radar takes its whole reference frame from `veh:GetForward()`/`GetRight()`, so a vortex dead ahead could report as `090 RIGHT`. A `HEADING_OFFSET_DEG` correction now rotates the vehicle-relative components into the nose's frame before the bearing text, blip and movement arrow consume them, so all three stay consistent. Defaults to `-90`, overridable for testing via the `TIV_HEADING_OFFSET_DEG` global.
