...
- **Airboats, Prisoner Pods and Seats No Longer Count as Interceptors**: `TIV.IsSupportedVehicle` ended with a bare `ent:IsVehicle() then return true`, so every vehicle on the map qualified -- airboats, prisoner pods, and the seats of unrelated jeeps, which GMod also gives the class `prop_vehicle_jeep`. `TIV.ResolveVehicle` then returned whichever one the player was sitting in, and the radar, HUD and instrument stream attached to it. Detection is now explicit: airboats are dropped from both `SupportedClasses` and `SupportedModelKeywords`, a new `ExcludedClasses` / `ExcludedModelKeywords` pair is checked before the tag and class lists so an exclusion wins even over an explicit `TIV.TagAsInterceptor`, and the `IsVehicle()` catch-all is gone.
- **Removed the Radar Movement Arrow**: The directional arrow showing where the vortex is travelling read as noise on the display, so the shaft, arrowhead wings and the `X MPH` badge beside it are gone. `SPEED` is still shown in the telemetry box, so nothing is lost. The vortex core and outer windfield arcs are unchanged.
- **Body Shift Added to the Visual Rocking**: Alongside the tilt, the model now shifts a couple of Source units downwind and squats into whichever anchors are still holding, which is the "shift slightly" half of the rocking behaviour. It uses `Entity:SetRenderOrigin`, read from `phys:GetPos()` and capped at 2.5 units, and is cleared with `nil` on the same settle path as the tilt. Because `SetRenderOrigin` shadows `GetPos()` and the radar blip reads `veh:GetPos()`, that interaction is measured rather than assumed: running the real `DrawRadarScreen` with the shift applied moves the blip by 0.0000 px at 12000u range. `tiv_visual_rock_shift 0` turns the origin override off and leaves the tilt.
- **Lofting State Now Feeds the Visual Stress**: The load model reads the signals the loft system already maintains -- `TIV.Loft.FailingGroups` and `data.gravityReleased` -- so a vehicle whose anchors have started shearing strains even if the wind momentarily drops below the stress floor, and the pivot term carries more weight while that is happening.
- **`ForceDetach` Restores Motion, Not Just Gravity**: It re-enabled gravity but left motion as it found it. The deploy sequence disables motion while it lerps the body down and back up, so a detach landing inside that window restored gravity on a body still not allowed to move -- it hangs there under its own weight. It now also enables motion and wakes the body. It only ever enables; it never freezes and never adds a constraint.

- **Visual Anchor Rocking, Render-Only**: An anchored TIV now visibly strains as its anchors load up -- nose lift into a headwind, a lean toward whichever side the wind is arriving from, both growing as anchors shear off. It is done with `Entity:SetRenderAngles` on the client and touches nothing else: no `SetPos`, no `SetAngles`, no `EnableMotion`, no new constraint. The physics body keeps sole responsibility for position, rotation, gravity and lofting, so this cannot freeze the vehicle the way the earlier physics-animation attempt did. The tilt is recomputed every frame as (physics angles + a bounded offset), read from the physics object rather than `GetAngles()` -- which the render override shadows, so reading it back would compound the offset every frame. When the server stops reporting, the model eases back to level over a couple of seconds and the override is cleared with `nil`, handing `GetAngles()` back to the real transform.
- **Anchor Load Model**: `tiv/anchor/sv_rock.lua` derives the lean from data the systems already have -- wind speed and direction, vehicle orientation, each anchor's mount position, and how many anchors are deployed versus sheared -- and broadcasts six bytes per anchored vehicle at 10 Hz, skipping the message entirely when nothing is anchored. Signs are measured against the Source basis rather than assumed (`+pitch` lifts the nose, `+roll` drops the right side), and the two contributions are kept distinct: wind peels the windward end up, while a sheared anchor lets the corner it was holding fly.
- **Handbrake Leak After a Loft**: `finalizeAnchored` applies the handbrake and `StartRetract` releases it, but `TriggerLoft` bypasses retract entirely -- so a vehicle lofted out of its anchors landed with the handbrake still on and would not drive, with perfectly healthy physics. It is now released on loft and again in the post-loft cleanup.
- **Freeze Audit and Idle Watchdog**: `tiv_debug_freeze 1` prints the real physics and constraint state of every TIV once a second -- motion/gravity/asleep flags, entity-vs-physics position drift (which is how you spot something calling `SetPos` every tick), constraint counts split by owner, spike and failure counts, live loft timers, lingering loft tracking and storm-immunity flags -- and flags the combinations that actually leave a vehicle hanging. `tiv_freeze_watchdog` (on by default) enforces one invariant: a TIV in `idle` is drivable. It only ever enables physics and only ever removes constraints this addon created, never freezes anything, and leaves vehicles mid-sequence completely alone.

- **Doppler Velocity Signature on the Radar**: While a vortex is actually on the ground the radar now paints a simplified tactical velocity couplet -- two opposing curved lobes marking the air moving toward the radar on one flank and away from it on the other, centred on the vortex and sized from the detected circulation. It is an additional layer: the outer windfield, inner core, centre marker, sweep, vehicle chevron and predicted track are all untouched, and the couplet is fitted into the annulus between the two existing circles so it cannot paint over either. Nothing about it rotates, so it cannot be mistaken for a spinning texture.
- **Ground Contact and Circulation Read From the Storm Addons, Never Assumed**: A new compatibility layer in `tiv/wind/sv_wind.lua` (`GetTornadoGroundContact`, `GetTornadoRotationDirection`, `GetTornadoRotationSpeed`, `GetTornadoCirculationReport`) reads what each addon actually publishes. Ground contact comes from GStorms' own funnel height -- `FunnelStartHeight` sits at `FunnelMaxHeight` while the funnel is aloft and is lerped to the ground on touchdown -- and from a downward trace for XT2/XT3, which anchor the vortex at the ground and publish no aloft state. Rotation comes from `Anticyclonic` (GStorms), `AntiCyclonic` (XT3) or `IsAnticyclonic` (XT2), falling back to the sign of XT2's `rotforce` for the EF variants that only flip that. Every branch returns *unknown* rather than a guess when an addon publishes nothing, and unknown ground contact is collapsed to `false` before it reaches the client -- so an active-but-aloft, unrecognised, stale or vanished vortex draws no signature at all, and an unreadable circulation direction draws a neutral single-colour couplet that claims no direction.
- **Circulation Telemetry**: `TIV_RadarPathData` gained three appended fields -- `touchingGround` (bool), `rotationDirection` (signed byte, -1/0/+1 for anticyclonic/unknown/cyclonic) and `rotationSpeed` (float MPH). They go out on the same ~0.35 s timer as everything else; the signature is interpolated client-side, never sent per frame.
- **Radar Heading Correction Set to +90**: A vortex visually dead ahead read as `REL BRG 090 RIGHT`. Measured against the real code, the direction that reads `090` at offset 0 reads `000` at offset +90, so `TIV.Config.HeadingOffsetDeg` now ships at 90.
- **Radar Blip Placed by Bearing, Not by Ad-Hoc Signs**: The blip's screen position is now a single 2D rotation of the unit vector toward the vortex, with one calibration knob (`TIV.Config.BlipOffsetDeg` / `tiv_radar_blip_offset`). The previous code carried hand-picked sign flips per axis, one of which -- negating the right component -- was wrong: the blip's horizontal axis was never broken, and the report that prompted it was about the `REL BRG` readout. The radar also paints both corrections as `HDG +90  BLIP +0` so they can be read off the screen.
- **Shared Relative-Bearing Helper**: `TIV.RelativeBearing(veh, targetPos)` in `tiv/config/sh_config.lua` is now the single source of truth for track-up bearing. The radar screen, the HUD and the Expression 2 functions all call it, so a chip-driven needle can no longer disagree with the built-in screen. It also fixes a normalisation bug present in all three: `atan2` can return exactly `-0.0`, `-0.0 < 0` is false, so the `+360` correction was skipped and the value fell through the sector chain into the `LEFT` branch -- a vortex dead ahead could read as LEFT. Normalisation is now `% 360`.
- **In-Game Heading Calibration**: `tiv_radar_heading_offset` sets the heading correction live, and the radar paints the value in force as `HDG OFF`. Resolution order is probe override, then convar, then `TIV.Config.HeadingOffsetDeg`.
- **Radar Heading Basis Correction**: `sh_custom_config.lua` documents the addon's vehicle-local space as `Forward: +Y (along veh:GetForward())`, but in GMod `Angle:Forward()` is local `+X` and `Angle:Right()` is local `-Y` -- the two halves of that line disagree by 90 degrees. The radar takes its whole reference frame from `veh:GetForward()`/`GetRight()`, so a vortex dead ahead could report as `090 RIGHT`. A heading correction rotates the vehicle-relative components into the nose's frame before the bearing text, blip and movement arrow consume them, so all three stay consistent. Its value is NOT yet determined: two field reports (offset 0 read `090 RIGHT` for a vortex visually ahead, offset -90 read `180 ASTERN`) are mutually inconsistent under any single fixed offset, so it ships at 0 pending an in-game measurement.
...
- **Every Upgrade Now Changes the Vehicle**: Four of the seven unlocks did nothing you could see. `heavy_cluster_spikes` accumulated a `max_spikes` bonus that no code ever read; `roof_spoiler` gated `armor_roof` components that no default config defined and the editor could not create; `reinforced_hydraulics` changed a stat but had no component type at all; and `path_screen` was silently dropped by the spawn lifecycle. Each one now puts something on the vehicle, verified by `tools/upgrade_visual_probe.lua`, which unlocks each upgrade in isolation and asserts a component appears.
- **Heavy Anchor Array Made Real**: `TIV.CustomConfig.CalculateVehicleStats` already emitted `max_spikes` (+2) and nothing consumed it -- the anchor count came from `TIV.Config.SpikeCount` clamped to `SpikeCountConvarMax`, which is 6. The new `TIV.Spikes.ResolveCount(veh)` is now the single answer to "how many anchors does this vehicle have", taking its ceiling from `max(SpikeCountConvarMax, stats.max_spikes)` and its value from how many spike mounts the vehicle's config actually defines. Both the creator (`CreateSpikes`) and the reconciler (`EnsureSpikes`) call it, which matters: they previously took the count from different sources and agreed only because every default defined exactly six. The two extra mounts (`spike_or` / `spike_ol`, outboard of the fronts and rears) are appended last in all five default configs, so without the upgrade the stock six-anchor geometry is untouched and the ceiling simply stops them being built.
- **Reinforced Hydraulic Rams Now Mount Physical Rams**: A new `hydraulic_ram` component type is gated on the upgrade, with two rams per default config and a curated model list in the editor. The stat half (`drive_depth_bonus` +6) was already live -- `sv_spike_anim.lua` adds it to `driveDepth` -- so this adds the visible part rather than changing the depth.
- **Roof Spoiler Gets a Roof Cowl**: Every default config now defines an `armor_roof` panel, so the upgrade that gated that type finally has something to gate.
- **Path Screen Actually Mounts the Screen**: `EnsureArmor` decided whether any components were wanted from `hasSideUpgrade or hasFrontUpgrade or hasRoofUpgrade` alone. A player who unlocked the Path Screen -- or the hydraulic rams -- and nothing else fell into the "no components wanted" branch, which removed any existing props and returned without spawning anything. The screen upgrade did nothing at all through the normal spawn and enter lifecycle. Both the screen and the rams are now part of that decision and of the component tally.
- **Armor Components No Longer Respawn on Every Call**: `EnsureArmor` keeps its own tally of what should exist and rebuilds everything when the tally disagrees with what is actually parented. That tally counted only the three armor types while `SpawnArmorProps` also built screens and rams, so the two disagreed and the reconciler tore the panels down and rebuilt them on every call. Measured across all 128 upgrade combinations by calling the real `EnsureArmor` twice on the same vehicle: **80 of 128 combinations respawned** before the fix, none after.
- **Editor Can Place Every Component Type**: `+ Add Armor` hardcoded `armor_side` and nothing in the UI ever wrote a component's `type`, so a roof cowl, screen or ram could not be created even once unlocked. The editor now has a component type selector (which also drives the curated model list) and `+ Add Roof`, `+ Add Screen` and `+ Add Hydraulics` buttons.
- **`CuratedModels` Keyed to the Wrong Names**: The list was keyed `front_armor` and `roof_armor` against components typed `armor_front` and `armor_roof`, and `spikes` against `spike`. The editor's `CuratedModels[curComp.type]` lookup therefore missed and silently fell back to the spike list, so armor components were offered harpoon models. Keys are renamed to match the component types, with `spike`/`spikes` aliased so either resolves.
- **Upgrade Visibility Harness**: `tools/upgrade_visual_probe.lua` runs the real `sh_progression.lua`, `sh_custom_config.lua`, `sv_custom_components.lua` and `sv_spikes.lua` against fake props. 30 checks: each upgrade alone yields a component or an anchor-layout change, `max_spikes` reaches the cap (6 → 8) while the first six mounts stay the original layout, the creator and reconciler agree for every upgrade combination, all 128 combinations are stable across repeated `EnsureArmor`, a screen-only unlock mounts the screen, and the curated list resolves for all six component types. Every fix in this batch was regression-proved by reverting it and watching the probe fail.

- **Saved Configs Now Get the Heavy Anchor Array Too**: The anchor ceiling comes from `max_spikes` but the count comes from how many spike mounts the *config* defines, and a config saved in the editor only defines the mounts it was saved with. Every layout anyone had already saved -- including the fully upgraded buggy exported from the 3D editor -- defines six, so the upgrade stayed invisible for exactly the players who had used the editor. Measured on that export: `max_spikes` 8, `ResolveCount` **6**. `GetSavedConfig` now runs `TIV.CustomConfig.MigrateSpikeMounts`, which brings a saved layout up to its model's factory mount count. The new mounts are placed from the saved config's own spike geometry -- outboard of its widest row, at that row's height, inheriting the angle convention of the spike already on that side -- so they land on the layout the player arranged rather than on factory coordinates, and an angled-spike vehicle does not get two vertical anchors bolted on. Migration is idempotent and leaves a config that already has eight untouched.
- **Component Placement Set From a Real Export**: The user aligned the roof cowl and hydraulic rams in the 3D editor and exported the result, so those positions are now measured rather than guessed. The roof was 29 units too low and stood on edge (`Angle(0, 90, 90)` with `metal_plate1x2`); it is now laid flat at `(0, -49.2, 79.0)` with `Angle(0, 0, 168.5)` and `metal_plate1`, and the rams moved to `(±35.9, -24.6, 30.0)`. The remaining four vehicle branches were re-placed by the same offsets expressed against each branch's own geometry -- roof at `(midY - 29.2, sideZ + 47.2)`, rams at `(± maxX + 5.9, midY - 4.6, sideZ - 1.8)` -- which reproduces the verified buggy coordinates exactly, so the transferable part is the offset, not a second guess. The spikes, side panels, front panel and screen in the export were already byte-identical to the factory defaults and are unchanged.
- **Export Pinned as a Regression Fixture**: `tools/fixtures/buggy_fully_upgraded.lua` holds that export. `tools/upgrade_visual_probe.lua` now loads it through the real `GetSavedConfig` and asserts both that migration turns its six mounts into eight and that the factory buggy layout still matches the exported coordinates, so a re-guessed roof or ram position fails the probe. 45 checks, up from 30.

- **Jalopy Layout Set From a Second Export**: `models/vehicle.mdl` now carries the coordinates from the user's exported fully upgraded jalopy -- roof cowl at `(0, -17.8, 67.5)` with `Angle(6.9, 90, 0)` and `metal_plate1x2`, rams at `(±35.0, 0, 30.0)`, and the two outboard mounts at `(±44.9, -135, 10.0)`. The second export also disproved the offsets the previous commit had fitted to the buggy alone: they predicted a jalopy roof at `z = 88.0` against an actual `67.5`, and a ram at `z = 39.0` against an actual `30.0`. The ram is at `z = 30.0` on **both** verified vehicles even though their side armour sits at `31.8` and `40.8`, so that height is absolute rather than `sideZ`-relative -- one data point fitted, the second falsified it.
- **The Two Roofs Are Different Parts**: The buggy carries a flat `metal_plate1` cowl at pitch 0; the jalopy carries a `metal_plate1x2` spoiler at `Angle(6.9, 90, 0)`. There is no single roof convention to transfer between models, so the APC, airboat and van/truck/pickup branches are back to being marked plainly as unverified guesses rather than being dressed up as derived values. The ram height (`z = 30`) is the one thing applied to them, because it is the only measured quantity that agrees across both verified vehicles.
- **Outboard Mounts Mirror by Yaw**: The exported jalopy tilts its left outboard mount with `Angle(80, 180, 0)` rather than the `Angle(100, 0, 0)` the inner six use. Both splay the spike the same way; the outer pair now has its own `rightOuterAng` / `leftOuterAng` so it keeps the exported orientation while still going to `Angle(90, 0, 0)` on every mount when `angled_spikes` is locked, which is what makes that upgrade visible.
- **Both Exports Pinned**: `tools/fixtures/jalopy_fully_upgraded.lua` joins the buggy fixture. `tools/upgrade_visual_probe.lua` now compares each factory branch against its export mount by mount -- position and angle for all eight anchors, plus roof, rams, side plates, front plate and screen -- and asserts the migration adds nothing to a config that already defines eight. 57 checks, up from 45; reverting the jalopy roof to the disproved formula value fails 3 of them.

- **Buggy's Extra Mounts Placed From the Corrected Export**: The user's second buggy export defines all eight mounts, and the two extra ones are an additional row at `(±25, 20)` -- between the front pair at `y = 50` and the mid pair at `y = -20`, at the front row's `x`. They are not outboard at `(±46, -20)`, which is where the factory default and the migration rule both put them. Both are now corrected, and the pair uses the ordinary `80`/`100` pitch convention rather than the jalopy's yaw mirroring, so `rightOuterAng` / `leftOuterAng` now apply to the jalopy branch alone.
- **Migration Copies the Factory Mounts Instead of Inventing Them**: `MigrateSpikeMounts` derived the missing mounts from the saved layout's geometry -- outboard of its widest row, at that row's height. Both exports disproved that: the verified buggy mounts are an extra row, and the verified jalopy mounts sit far behind the rear axle. Each model's extra mounts are bespoke, so the rule is now to append that model's own trailing factory mounts, which are verified for the buggy and the jalopy and at least consistent with the factory everywhere else. The mounts a player already placed are untouched.
- **Two Buggy Fixtures**: `buggy_fully_upgraded.lua` is now the corrected eight-mount export; the earlier six-mount one is kept as `buggy_six_mounts.lua`, since it is the case the migration exists for. `tools/upgrade_visual_probe.lua` asserts the migrated mounts equal the buggy's verified factory mounts rather than any computed position, and that the six mounts the player placed keep their coordinates. 55 checks; reverting the buggy mounts to `(±46, -20)` or restoring the geometric migration rule each fails one.

- **APC Layout Set From a Third Export**: `models/combine_apc.mdl` now carries the exported coordinates. Four parts were wrong: the radar screen moves from `(15, 52, 50)` to `(-8, -0.1, 80)` with `Angle(-10, -75, 0)`, the roof from a guessed flat cowl at `(0, -19.2, 96.4)` to `(0, 75.9, 80.0)` with `Angle(166.2, -90, 0)` and `metal_plate1x2`, the ram from `y = 10` to `y = 0`, and the two extra mounts from outboard at `(±51, 10)` to `(±25, -20)`. The side panels and front plate were already right.
