...
The probes validate the radar's own maths and the painted coordinates. They do
**not** validate the assumption that `cam.Start3D2D(pos, screenAng, scale)` maps
canvas +x to `screenAng:Forward()` and canvas +y to `screenAng:Right()` — that
needs a rendered frame, i.e. a real GMod client.
